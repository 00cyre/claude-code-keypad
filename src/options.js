...
import { CREATOR_MICRO_V2_IDS } from "./keypad.js";
...
export const DEFAULT_OPTIONS = {
  ...
  onlyOnLayer: false,
  assumeYes: false,
  // Only open the Creator Micro 2. Without a filter the bridge opens the first
  // Work Louder device it finds, so on a Mac that also has a Codex Micro
  // (0x8360) this would grab it whenever the Creator Micro 2 is asleep and
  // paint Claude colours over the Codex ones. Two ids because the same keypad
  // enumerates as either; --product-id any restores the old behaviour.
  productIds: CREATOR_MICRO_V2_IDS,
};
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--product-id": {
        const v = next();
        if (v === "any") {
          options.productIds = null;
          break;
        }
        const id = Number(v);
        if (!Number.isInteger(id) || id <= 0) {
          throw new Error(`--product-id wants a number like 0x8298, or any — got ${v}`);
        }
        options.productIds = [id];
        break;
      }
